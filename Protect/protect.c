...
void Protect_Check_Slow()
{
    ...
    {
        if (config_data.fila_protect_cnt[i] > para_range.fila_protect_cnt_max)
        {
            mHVPS_Fault.FAULT_REG4.bit.lamp_wait_overtime = 1;
        }

        /* 单次曝光超限定时间 */
        if (config_data.expo_count[i] > para_range.expo_time_limit)
        {
...
void UpdateVar_CheckFaultFast()
{
    ...
    float tube_vol_target = Is_CalibrateMode() ? CALI_HV_REF : (float)config_data.tube_vol[ctrl_data.xray_current - 1];
