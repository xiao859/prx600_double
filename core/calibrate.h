...
#define     Is_PulseMode()                  ((cali_data.mode) == XRAY_MODE_S_PULSE)

#define     Is_ContinuousMode()             ((cali_data.mode) == XRAY_MODE_S_CONTINUOUS)

/* 3、数据类型定义 */
typedef struct {
    xray_mode   mode;                   /* 校准模式：脉冲0或者连续1 */
    uint8_t     curr_index[XRAY_NUMS];             /* 校准阶段0~9代表 */
    uint8_t     finished_flag[XRAY_NUMS];          /* 结束标记 */
