/*
 *  CT_mode.c
 *
 *  Created on: Feb 24, 2025
 *  Author: Administrator
 *
 */

#include "exposure.h"
#include "calibrate.h"
#include <stdio.h>
#include "comm_string.h"
#include "comm_protocol.h"
#include "xray.h"
#include "delay.h"
#include "pi_control.h"
#include "app_uart.h"

volatile xray_running_data xray_data = {0};

/* CT 模式关闭放线 */
void xray_CT_disable()
{
    /* 基准清零 */
    disable_hvref();
    disable_filamentref();

    /* 给射源 */
    config_HVEn_signal(0);      /* 高电平开 */
    config_mcuLock_signal(0);

    /* 给灯丝 */
    config_filamentOn_signal(0);

    /* 给主控 */
    config_ready_signal(0);
    config_xrayOn_signal(0);

    return;
}

float kp_test = 3.5;
float ki_test = 0.0009;
extern uint32_t test_flag;
void ct_task()
{
    if (xray_data.timmer_count >= 1) xray_data.timmer_count++;

    /* 异常场景，立马关闭 xray_CT_disable(); */

    if (get_hv_state() == HVPS_SM_ID_IDLE) {
        hvState_ilde_init();
        /* 开启灯丝后，需要预热2.5秒 */
        if ((ctrl_data.enable == 1) && (ctrl_data.interlock == 1) &&
            (ctrl_data.filament_on == 1) && (xray_data.timmer_count > TIMER6_2P5_SECOND_CYCLES)) {
            config_mcuLock_signal(1);
            set_hv_state(HVPS_SM_ID_PREPARE);
            xray_data.timmer_count = 1;
            config_data.expo_count = 0;
        }

        if (ctrl_data.filament_on == 1) {
            config_filament_ref_slop();
        }

    } else if (get_hv_state() == HVPS_SM_ID_PREPARE) {
        config_filamentRef();     /* 灯丝基准拉到预期值 */

        /* 通知MCU已准备好 */
        config_ready_signal(1);
        set_hv_state(HVPS_SM_ID_READY);
        xray_data.timmer_count = 0;
        uint32_t currRef = config_data.fila_ref_realtime;

        pid_Init(config_data.tube_curr, currRef, Is_PulseMode_CT());

    } else if (get_hv_state() == HVPS_SM_ID_READY) {
        /* 计时判断 */
        config_hvref_slope();
        if (ctrl_data.enable && ctrl_data.expo) {
            /* 给高压射源 */
            config_HVEn_signal(1);

            /* 通知MCU正在曝光 */
            config_xrayOn_signal(1);

            set_hv_state(HVPS_SM_ID_EXPOSURING);
            xray_data.timmer_count = 1;
        }

        /* EXP先关，ENABLE后关 */
        if (ctrl_data.enable == 0) {
            if (config_data.expo_count_total > 1) {
                parm_table.expo_count_total++;
                /* 曝光60s加一次 */
                parm_table.expo_times_total += config_data.expo_count_total / 3000000;
            }
            config_xrayOn_signal(0);
            set_hv_state(HVPS_SM_ID_EXPO_END);
        }

    } else if (get_hv_state() == HVPS_SM_ID_EXPOSURING) {
        config_hvref_slope();

        uint32_t pulse_timer = Is_ContinuousMode_CT() ? TIMER6_8_MILSECOND_CYCLES : TIMER6_8_MILSECOND_CYCLES;

        if (xray_data.timmer_count == pulse_timer) {
            user_pid.currValue = sampled_data.tube_curr_value;
        }

        /* 连续模式下，每隔10ms调一次 */
        // if (Is_ContinuousMode_CT() && (xray_data.timmer_count > TIMER6_8_MILSECOND_CYCLES)) {
        if ((xray_data.timmer_count > TIMER6_5_MILSECOND_CYCLES)) {
            xray_data.isCheckAvailable = 1;
           if (xray_data.timmer_count % TIMER6_10_MILSECOND_CYCLES == 0) {
               user_pid.currValue = sampled_data.tube_curr_value;
               user_pid.Kp = kp_test;
	           // user_pid.Ti = 0.005;
               user_pid.Ti = ki_test;

                tube_current_piControl(1);
           }
        } else {
            xray_data.isCheckAvailable = 0;
        }

        /* ENABLE先关，EXP后关 */
        if (ctrl_data.enable == 0) {
            xray_data.isCheckAvailable = 0;
            parm_table.expo_count_total++;
            /* 曝光60s加一次 */
            parm_table.expo_times_total += config_data.expo_count_total / 3000000;
            config_xrayOn_signal(0);
            set_hv_state(HVPS_SM_ID_EXPO_END);
        }

        if (ctrl_data.enable && (ctrl_data.expo == 0)) {
            xray_data.isCheckAvailable = 0;
            config_xrayOn_signal(0);
            set_hv_state(HVPS_SM_ID_READY);
            user_pid.Kp = 20;
	        user_pid.Ti = 1;
            tube_current_piControl(0);
            debug_tx3("闭环值: %f, %d, %f, %f\n",
                param_pid.Out_pid, param_pid.config_ref, user_pid.currValue, param_pid.integral);
        }

        if (ctrl_data.enable && ctrl_data.expo) {
            /* 曝光计数和超时判断 */
            config_data.expo_count_total++;
            config_data.expo_count++;
        }

    } else if (get_hv_state() == HVPS_SM_ID_EXPO_END) {
        ctrl_data.filament_on = 0;
        xray_CT_disable();
        cali_data.para_save_flag = 1;

        set_hv_state(HVPS_SM_ID_IDLE);
    }

    (get_hv_state() == HVPS_SM_ID_EXPOSURING) ? xray_on_led(1) : xray_on_led(0);

    if (get_filament_pin() && (get_hv_state() != HVPS_SM_ID_EXPOSURING)) {
        config_data.fila_protect_cnt++;
    } else {
        config_data.fila_protect_cnt = 0;
    }

    return;
}
