...
{
    if (Is_PulseMode()) {
        cali_data.expoCycle_perCurrent = (uint32_t)((CALI_SINGLE_CURR_TIME * 1000) / CALI_PULSE_SIGLE_CURR_PERIOD);
        ...
        cali_data.expoCycle_perCurrent = 1;
...
{
    cali_data.mode = XRAY_MODE_PULSE;
    ...
    config_data.fila_ref_realtime = 0;
    config_data.fila_ref_step = IDLE_FILAMENT_REF / (20 * 50);          /* 20ms上升时间 */
...
{
    if (Is_PulseMode()) {
        parm_table.currRef[curr_idx]   = param_pid.config_ref;
    } else {
        parm_table.currRef_c[curr_idx] = param_pid.config_ref;
    }
...
void config_filamentRef_cali(uint8_t curr_index)
{
    uint32_t fila_vol_ref = (Is_PulseMode()) ?
        parm_table.currRef[curr_index] : parm_table.currRef_c[curr_index];
...
{
    if (get_hv_state() == HPVS_SM_ID_CAL_EXPOSURING) {
    ...
    uint32_t tube_vol_ref = (uint32_t)((cali_data.tube_vol_realtime / 64) / ADDA_FULL_SCALE_VIL_VALUE * 4095);  /* 0~2.5V 对应 0~160kV */
...
void calibrate_task()
{
    if (cali_data.timmer_count >= 1) cali_data.timmer_count++;
    ...
    if (get_hv_state() == HPVS_SM_ID_CAL_PREPARE) {
        ...
        config_filament_ref_slop();

        /* 灯丝预热2.5秒 */
        if (cali_data.timmer_count > TIMER6_2P5_SECOND_CYCLES) {
            set_hv_state(HPVS_SM_ID_CAL_RUN);
            cali_data.timmer_count = 1;
            /* PID系数重置，注意每次开始RUN都需要重置，避免上次的遗留 */
            uint32_t currRef = (Is_PulseMode()) ?
                parm_table.currRef[cali_data.curr_index] : parm_table.currRef_c[cali_data.curr_index];
            pid_Init(parm_table.currValue[cali_data.curr_index], currRef, Is_PulseMode());
        }

    } else if (get_hv_state() == HPVS_SM_ID_CAL_RUN) {
        /* 配置灯丝电源和高压电源 DAC值 */
        config_hvref_cali();
        config_filamentRef_cali(cali_data.curr_index);

        /* 每个电流间需要有5秒的间隔时间 */
        if (cali_data.timmer_count >= TIMER6_5_SECOND_CYCLES) {
            // xray_HV_enable(1);  /* 开始曝光 */
            set_hv_state(HPVS_SM_ID_CAL_EXPOSURING);
            cali_data.timmer_count = 1;
        }
    } else if (get_hv_state() == HPVS_SM_ID_CAL_EXPOSURING) {
        xray_HV_enable(1);  /* 开始曝光，曝光和基准一起给 */
        config_hvref_cali();
        config_data.expo_count_total++;
        /* 曝光计时 */
        if (cali_data.timmer_count >= cali_data.expoTime_expect) {
            set_hv_state(HPVS_SM_ID_CAL_COOLING);
            cali_data.timmer_count = 1;
        }

        // if (cali_data.timmer_count == (uint32_t)(cali_data.expoTime_expect * 2 / 3)) {
        if (cali_data.timmer_count == 650) {
            user_pid.currValue = sampled_data.tube_curr_value;
            // debug_tx3("管电流: %f, %f\n", user_pid.currValue, sampled_data.tube_curr_value);
        }

        if (cali_data.timmer_count >= TIMER6_5_MILSECOND_CYCLES) {
        ...
        /* 连续模式下，每隔10ms调一次 */
        if (Is_ContinuousMode() && (cali_data.timmer_count > TIMER6_4_MILSECOND_CYCLES)) {
            if (cali_data.timmer_count % TIMER6_10_MILSECOND_CYCLES == 0) {
                user_pid.currValue = sampled_data.tube_curr_value;
                user_pid.Kp = 3.7;
	            user_pid.Ti = 0.0009;
    ...
    } else if (get_hv_state() == HPVS_SM_ID_CAL_COOLING) {
        config_hvref_cali();
        /* 脉冲之间的时间 */

        if ((Is_PulseMode()) &&
            (cali_data.timmer_count == (uint32_t)(cali_data.coolTime_expect / 2))) {
            user_pid.Kp = 20;
	        user_pid.Ti = 1;
        ...
        if (cali_data.cycle_count >= cali_data.expoCycle_perCurrent) {
            filament_ref_update(cali_data.curr_index);
            ...
            if (cali_data.curr_index >= FILAMENT_CURRENT_TABLE_ORDER) {
            // if (cali_data.curr_index >= 1) {
                /* 上报本次的校准结果 */
                uint32_t last_idx = cali_data.curr_index - 1;
                debug_tx3("闭环结果: %f, %d, %d\n",
                    parm_table.currValue[last_idx], parm_table.currRef[last_idx], param_pid.config_ref);
                cali_data.curr_index = 0;

                /* 该模式下所有电流校准完毕，脉冲或连续 */
                if (Is_PulseMode()) {
                    next_cal_state = HPVS_SM_ID_CAL_RUN;
                    cali_data.mode = XRAY_MODE_CONTINUOUS;
                    calibrate_mode_config();
                    uint32_t currRef = (Is_PulseMode()) ?
                        parm_table.currRef[cali_data.curr_index] : parm_table.currRef_c[cali_data.curr_index];
                    pid_Init(parm_table.currValue[cali_data.curr_index], currRef, Is_PulseMode());
        ...
        cali_data.timmer_count = 1;
    } else if (get_hv_state() == HPVS_SM_ID_CAL_END) {
        /* 记录校准数据 */
    ...
    (get_hv_state() == HPVS_SM_ID_CAL_EXPOSURING) ? xray_on_led(1) : xray_on_led(0);
